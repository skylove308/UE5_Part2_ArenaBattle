...
AABStageGimmick::AABStageGimmick()
{
	...
	// 플레이어가 특정 영역에 들어왔을 때 이벤트를 트리거하도록 트리거 박스 생성
	...
	{
		// 게이트의 절대 위치는 에디터에서 미리 정의. 이름으로 찾아서 불러옴
		...
		// OnComponentBeginOvelap 호출 시 OnGateTriggerBeginOver함수 실행
	...
	// State Section
	// 각 델리게이트를 Wrapper에 감싸 리스트로 넣어줌.
	// 델리게이트가 호출 될때 긱 스테이지의 기믹이 실행
...
// 액터가 생성되거나 에디터에서 속성이 변경될 때 호출, 액터의 초기화 또는 상태 업데이트에 사용
...
// 델리게이트 리스트에서 State에 따라 맞는 함수 호출
